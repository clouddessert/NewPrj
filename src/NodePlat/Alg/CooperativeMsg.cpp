	...
	{   
		...
		for ( iteBackMsg = vctBackCooperative.begin(); iteBackMsg != vctBackCooperative.end(); iteBackMsg++)
		{
			...
			if (iteBackMsg->lAutonum == iteRequestMsg->lAutonum)  //综合批号相同
			{
				...
				{	
					//转换航迹信息，将坐标，经纬度替换
									stSelfPosi.dLati = 120*PI/180;               //返回信息自身的B舰的地理经度（度），转成弧度
									stSelfPosi.dLonti = 20*PI/180;                //地理纬（度），转成弧度
									stSelfPosi.dHeight = 0;                        //米				
									iteRequestMsg->stReqShipPosi.dLati = 123*PI/180;               //本舰即请求舰A舰的地理经度（度），转成弧度
