...
void CDialogFUN::AddToGrid()
{
	...
	theApp.m_DataBase.pRst=theApp.m_DataBase.pConn->Execute("select * from ¹¦ÄÜÈº order by ID desc ",NULL,adCmdText);
	int type,trackbaseevent,funbaseevent;
	CString stype,strackbaseevent,sfunbaseevent;
	while(!theApp.m_DataBase.pRst->adoEOF)
	{
		m_Grid.InsertItem(0,"");
		type=atoi((_bstr_t)theApp.m_DataBase.pRst->GetCollect("ÈºÀàÐÍ"));
		trackbaseevent=atoi((_bstr_t)theApp.m_DataBase.pRst->GetCollect("Æ½Ì¨Ì¬ÊÆ"));
		funbaseevent=atoi((_bstr_t)theApp.m_DataBase.pRst->GetCollect("¹¦ÄÜÊÂ¼þ"));
...
/*	pConn.CreateInstance("ADODB.Connection");
	...
	*/
	//theApp.m_DataBase.pRst.CreateInstance(__uuidof(Recordset));
	theApp.m_DataBase.pRst->Open("select * from ¹¦ÄÜÈº",theApp.m_DataBase.pConn.GetInterfacePtr(),adOpenDynamic,adLockOptimistic,adCmdText);
	int type,trackbaseevent,funbaseevent;
	CString stype,strackbaseevent,sfunbaseevent;
	...
	try
	{
		...
		theApp.m_DataBase.pRst->PutCollect("ÈºÀàÐÍ",(long)type);
		theApp.m_DataBase.pRst->PutCollect("Æ½Ì¨Ì¬ÊÆ",(long)trackbaseevent);
		theApp.m_DataBase.pRst->PutCollect("¹¦ÄÜÊÂ¼þ",(long)funbaseevent);
